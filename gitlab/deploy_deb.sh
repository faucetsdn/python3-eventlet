...
for release in $(awk -F ',' -v today="$(date --utc "+%F")" \
    ...
    package_cloud push faucetsdn/faucet-test/ubuntu/${release} *.deb < /dev/null || true
done
...
for release in $(awk -F ',' -v today="$(date --utc "+%F")" \
    ...
    package_cloud push faucetsdn/faucet-test/debian/$release *.deb < /dev/null || true

    echo "==== Uploading packages to raspbian/${release} ==="
    package_cloud push faucetsdn/faucet-test/raspbian/$release *.deb < /dev/null || true
done
